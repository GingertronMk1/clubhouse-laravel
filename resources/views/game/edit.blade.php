<x-app-layout>
    <form action="{{ route('game.update', ['game' => $game]) }}" method="post">
        ...
        <x-input name="competition_id" type="select" :options="\App\Models\Competition::get()" />
        <x-input name="team_1_id" type="select" :options="\App\Models\Team::get()" />
        <x-input name="team_2_id" type="select" :options="\App\Models\Team::get()" />
